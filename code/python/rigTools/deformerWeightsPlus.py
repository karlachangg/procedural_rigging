...
Karla Chang To Do:
-add logging and convert print statements
...
class SkinDeformerWeights(object):
    def __init__(self, path=None):
        ...
        self.shapes = {}
        self.fileName = None
        # call parseFile function to set the class object's joint, shape, skin, and verts
        if self.path:
    ...
    def applyWeightInfo(self, worldSpace=False, normalize=False, debug=False):
        try:
            for shape in self.shapes:
                ...
                #print('Reading the shape in the class dictionary of shape objects. Shape is: {}'.format(shape))

                # make a skincluster using the joints
                if cmds.objExists(shape):
                    ss = self.shapes[shape]

                    #print('Get value using dictionary key. ss is: {}'.format(ss))

                    skinList = ss.joints

                    #print('joints attribute of ss object: {}'.format(skinList))

                    # Create new list of joint influences after checking they exist in the scene
                    newSkinList = [j for j in skinList if cmds.objExists(j)]

                    #print('newSkinList: {}'.format(newSkinList))
                    ...
                    # Add shape to newSkin list. Now it is a list with all joints + the affected mesh
                    ...
                    # Removing this code for now. If we lock the joints, we cannot edit the skinweights.
                    '''
                    for obj in lockedNodes:
                        print('Obj in lockedNodes list: {}'.format(obj))
    ...
    def saveWeightInfo(self, fpath, meshes, all=True):
        ...
        deformers = list(meshDict.keys())

        if mayaVer > 2016:
            attributes = ['envelope', 'skinningMethod', 'normalizeWeights', 'deformUserNormals', 'useComponents']
            cmds.deformerWeights(fname, path=dir, ex=1, vc=1, attribute=attributes, deformer=deformers)
